		...
		if (hoten != null && email != null && diachi != null && sdt != null) {
			try {
				...
				if (kh != null) {
					...
					long themhoadon = hdbo.themhoadon(kh.getMakh(), ngay);
					long mahoadon = hdbo.maxmahd();
					giohangbo gh = (giohangbo) session.getAttribute("gio");
					khachhangbean khachhang = hdbo.getkhachhang(kh.getEmail());
					for (giohangbean item: gh.ds) {
						long themchitiethoadon = hdbo.themchitiethoadon(item.getMasp(), (long)item.getSoLuong(), mahoadon);
					}
